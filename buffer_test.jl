...
function distance_calculation()
    ...
    global spatial_distance = Vector{Any}()
    for rows in 1:N_sg
        append!(spatial_distance, distance_ij[(rows+1):N_sg, (rows)])
    end
end
...
function calculate_spatial_correlation()
    ...
    global spatial_corltn = Vector{Any}()
    for rows in 1:N_sg
        append!(spatial_corltn, corltn_term[(rows+1):N_sg, (rows)])
    end
end
...
global corltn_term_sum = zeros(N_sg*replica_num, N_sg) |> CuArray                  #<sigma_i*sigma_j>
...
scatter(spatial_distance, spatial_corltn)
#------------------------------------------------------------------------------------------------------------------------------#
