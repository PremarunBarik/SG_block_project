...
for i in eachindex(Temp_values)                               #TEMPERATURE LOOP 
    ...
    global mag = 0.0
    global en = 0.0
    global spin_av_per_replica = zeros(1, replica_num)
    ...
    magnetisation[Temp_index] = sum(spin_av_per_replica)/(replica_num*MC_steps)
end

#------------------------------------------------------------------------------------------------------------------------------#

#SAVING THE GENERATED DATA
open("2D_EA_$(n_x)x$(n_y)_KMC_B$(B_global).txt", "w") do io 					#creating a file to save data
