...
function spatial_correlation_terms(N_sg, replica_num)
    ...
    corltn = reshape(x_dir_sg, (N_sg, replica_num))' |>  Array
    corltn = repeat(corltn, inner = (N_sg, 1))                              #Scalar indexing - less time consuming in CPU
    corltn = corltn |> CuArray
    corltn = corltn .* diag_zero

    global corltn_term1 += x_dir_sg .* corltn                                      #<sigma_i*sigma_j>
   
    term2 = repeat(x_dir_sg, 1, N_sg)
    global corltn_term2 += term2 .* diag_zero                                      #<sigma_i>

    global corltn_term3 += corltn                                                  #<sigma_j>
end
...
@CUDA.allowscalar for i in eachindex(Temp_values)                              #TEMPERATURE LOOP 
    ...
    global corltn_term2 = zeros(N_sg*replica_num, N_sg) |> CuArray                #<sigma_i>
    global corltn_term3 = zeros(N_sg*replica_num, N_sg) |> CuArray                #<sigma_j>
...
open("SG_EA_Mag_SpCorrln_10_10_3.txt", "w") do io 					#creating a file to save data
   for i in 1:length(Temp_values)
      println(io,i,"\t",Temp_values[i],"\t",spatial_correlation[i],"\t",magnetisation[i])
   end
end

plot(Temp_values, spatial_correlation, xlabel="Temperature(T)", ylabel="Spatial Correlation (dc susceptibility)")
savefig("SpatialCorrlnVsTemp_SG_EA.png")

plot(Temp_values, magnetisation, xlabel="Temperature(T)", ylabel="Magnetization/spin")
savefig("MagnetizationVsTemp_SG_EA.png")
