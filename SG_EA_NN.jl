    ...
    for j in i:N_sg
        for k in 1:replica_num
            if i==j
                ...
                J_NNN[i,j,k] = J_NNN[j,i,k] = 0                                   #for ising: 1, for spin glas: random
            end
        end
    end
end
...
@CUDA.allowscalar for i in eachindex(Temp_values)                              #TEMPERATURE LOOP 
    ...
    global rand_pos = CuArray(rand(rng, (1:N_sg), (replica_num, MC_burns)))                 #generating same set of random number everytime
    #MATRIX WITH RANDOM FLOAT NUMBER TO FLIP SPIN
    global rand_num_flip = CuArray(rand(rng, Float64, (replica_num, MC_burns)))             #generating same set of random numbers everytime.
...
open("SG_EA_eaOrdr_10_10_3_reproduce.txt", "w") do io 					#creating a file to save data
   for i in 1:length(Temp_values)
      println(io,i,"\t",Temp_values[i],"\t",magnetisation[i],"\t",EA_order_para[i])
   end
end
